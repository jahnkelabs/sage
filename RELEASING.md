...
4. **`workflow_dispatch`**: in **Actions → Release → Run workflow**, use the branch/tag dropdown: pick **`main`** for a normal **`semantic-release` + maybe GoReleaser** run; pick a **`refs/tags/v*`** tag to run **GoReleaser only** for that exact version (no **`semantic-release`** on tag refs).
